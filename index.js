...
// ==========================================
// 🔥 NEW: AUTO MENTION / TAG SYSTEM HELPERS
// ==========================================
    ...
    if (taskInfo.targetType === "group" && taskInfo.mentionType && taskInfo.mentionType !== 'none') {
        ...
        if (taskInfo.mentionType === 'all_tag') {
            ...
            try {
                ...
                logError(`Failed to fetch group metadata for mentions: ${e.message}`);
            }
        ...
        while (taskInfo.isSending && !taskInfo.stopRequested) {
            ...
            try {
                ...
                const successLog = {
                    ...
                    details: `To: ${taskInfo.target} | Message: "${finalMsg.substring(0, 50)}${finalMsg.length > 50 ? '...' : ''}"`,
                    timestamp: new Date()
                };
        ...
        while (taskInfo.isSending && !taskInfo.stopRequested) {
            ...
            try {
                ...
                const successLog = {
                    ...
                    details: `To: ${taskInfo.target} | ${finalCaption ? 'Caption: "' + finalCaption.substring(0, 50) + '"' : 'No caption'}`,
                    timestamp: new Date()
                };
                logs.unshift(successLog);
                if (logs.length > 100) logs.pop();
...
app.post("/send-message", requireAuth, upload.single("messageFile"), async (req, res) => {
    ...
    if (!allowedMentionTypes.includes(mType)) {
        return res.json({ success: false, error: "Invalid mention type" });
    }
    ...
    if ((mType === 'single' || mType === 'all_listed' || mType === 'selected_group') && parsedMentionNumbers.length === 0) {
        return res.json({ success: false, error: "Mention numbers are required for the selected mention mode" });
    }
    if ((mType === 'all_group' || mType === 'selected_group' || mType === 'all_tag') && targetType !== "group") {
        return res.json({ success: false, error: "Group mention modes require target type to be 'group'" });
    }
    ...
    try {
        ...
        const taskInfo = {
            ...
            logs: [],
            mentionType: mType,
            mentionNumbers: parsedMentionNumbers
        };
...
app.listen(PORT, () => {
    ...
    logSuccess('✅ AUTO MENTION / TAG SYSTEM enabled');
});
